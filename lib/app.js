...
app.get('/location', async (req, res) => {
  try {
    const userInput = req.body.location;

    const returnedObject = await fetch.get(`https://us1.locationiq.com/v1/search.php?key=${locationid_key}&q=${userInput}&format=json`);
    const parsedReturnedObject = JSON.parse(returnedObject.text);

    const objectToReturn = {
      formatted_query: parsedReturnedObject[0].display_name,
      latitude: parsedReturnedObject[0].lat,
      longitude: parsedReturnedObject[0].lon
    };
